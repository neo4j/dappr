...
class Selector:
    ...
    def recall(self):
        return len(self.pairs.intersection(self.embeddings.test)) / len(self.embeddings.test) if len(self.embeddings.test) > 0 else 0

    def precision(self):

        return len(self.pairs.intersection(self.embeddings.test)) / len(self.pairs) if len(self.pairs) > 0 else 0
