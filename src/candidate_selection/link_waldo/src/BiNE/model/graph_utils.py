...
class GraphUtils(object):
    ...
    def calculate_centrality(self, mode='hits'):
        ...
        for node_id, node_data in self.G.nodes(data=True):
            if node_data["bipartite"] == 0:
                if max_a_u < a[node_id]:
                    max_a_u = a[node_id]
                if min_a_u > a[node_id]:
                    min_a_u = a[node_id]
            if node_data["bipartite"] == 1:
                if max_a_v < a[node_id]:
                    max_a_v = a[node_id]
                if min_a_v > a[node_id]:
                    min_a_v = a[node_id]

        for node_id, node_data in self.G.nodes(data=True):
            if node_data["bipartite"] == 0:
                if max_a_u-min_a_u != 0:
                    self.authority_u[node_id] = (float(a[node_id])-min_a_u) / (max_a_u-min_a_u)
                else:
                    self.authority_u[node_id] = 0
            if node_data["bipartite"] == 1:
                if max_a_v-min_a_v != 0:
                    self.authority_v[node_id] = (float(a[node_id])-min_a_v) / (max_a_v-min_a_v)
                else:
                    self.authority_v[node_id] = 0
    ...
    def homogeneous_graph_random_walks_for_large_bipartite_graph_without_generating(self, G, percentage, maxT, minT):
        self.G_u, self.walks_u = self.get_random_walks_restart_for_large_bipartite_graph_without_generating(G, self.authority_u, percentage=percentage, maxT=maxT, minT=minT, node_type=0)
        self.G_v, self.walks_v = self.get_random_walks_restart_for_large_bipartite_graph_without_generating(G, self.authority_v, percentage=percentage, maxT=maxT, minT=minT, node_type=1)
    ...
    def get_random_walks_restart_for_large_bipartite_graph_without_generating(self, G, hits_dict, percentage, maxT, minT, node_type=0):
        # G = graph.load_edgelist(datafile, undirected=True)
        graph_obj = graph.from_networkx(G, undirected=True)
        # G = nx.read_edgelist(datafile)
        cnt = 0
        for node_id, node_data in G.nodes(data=True):
            if node_data["bipartite"] == node_type:
                cnt += 1
        print("number of nodes: {}".format(cnt))
        print("walking...")
        walks = graph.build_deepwalk_corpus_random_for_large_bibartite_graph(G, graph_obj, hits_dict, percentage=percentage, maxT = maxT, minT = minT, alpha=0, node_type=node_type)
        # print(walks)
        print("walking...ok")
        return graph_obj, walks
