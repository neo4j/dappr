...
def build_deepwalk_corpus_random_for_large_bibartite_graph(G, graph, hits_dict, percentage, maxT, minT, alpha=0, rand = random.Random(), node_type=0):
  walks = []
  nodes_total = list(G.nodes(data=True))
  nodes = []
  # print(len(nodes),node_type)
  for node_id, node_data in nodes_total:
  ...
  for node in nodes:
    ...
    # print num_paths,
    for cnt in range(num_paths):
     walks.append(graph.random_walk_restart_for_large_bipartite_graph(nodes, percentage,rand=rand, alpha=alpha, start=node))
  random.shuffle(walks)
  return walks
...
def from_networkx(G_input, undirected=True):
    ...
    for idx, (node_id, node_data) in enumerate(G_input.nodes(data=True)):
        for y in iterkeys(G_input[node_id]):
