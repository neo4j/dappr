...
class MGSelector(LinkWaldoSelector):
    ...
    def setup(self):
        ...
        if self.SG:
            ...
            struc_embeddings = XNetMF(self.G, self.embeddings.test_edges, emb_path, self.embeddings.G, normalize=False)
        ...
        if self.CG:
            ...
            if self.embeddings.name != 'netmf1' and self.embeddings.name != 'bine':
                ...
                clus_embeddings = NetMF(self.G, self.embeddings.test_edges, emb_path, self.embeddings.G, normalize=False, window_size=1)
                if not os.path.exists(emb_path):
